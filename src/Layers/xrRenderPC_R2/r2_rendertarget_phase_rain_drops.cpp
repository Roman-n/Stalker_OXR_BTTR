...
{
    float wetness = g_pGamePersistent->Environment().eff_Rain->GetCurrViewEntityWetness();
    ...
    if (wetness < rain_drops_const)
    {
        return;
    }
    else
    {   // oldSerpski stalker
        // если актер в шлеме -> включить появление капель 
        //if (helmet || !outfit->bIsHelmetAvaliable) // Мы или в шлеvе показать капли, или в костюме который со встроенным шлемом тоже показать капли
        //{
            u32 Offset = 0;
            Fvector2 p0, p1;
            ...
            RCache.set_c("rain_drops_params0", wetness, ps_r2_rain_drops_debug_control, 0, 0);
