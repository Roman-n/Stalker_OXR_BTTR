...
void CUIHudStatesWnd::InitFromXml(CUIXml& xml, LPCSTR path)
{
    ...
    if (__type_hud_cop || __type_hud_coc || __type_hud_soc || __type_hud_veter_vremeni) // для лост альфы отключаем
    {
...
void CUIHudStatesWnd::UpdateHealth(CActor* actor)
{
    ...
    if (__type_hud_soc || __type_hud_veter_vremeni)
    {
...
void CUIHudStatesWnd::UpdateIndicatorType(CActor* actor, ALife::EInfluenceType type)
{
    ...
    if (__type_hud_cop || __type_hud_soc || __type_hud_veter_vremeni || __type_hud_lost_alpha)
    {
        if (type < ALife::infl_rad || ALife::infl_psi < type)
        {
            VERIFY2(0, "Failed EIndicatorType for CStatic!");
        ...
        switch (type)
        {
        case ALife::infl_rad: texture = "ui_inGame2_cop_triangle_Radiation_"; break;
        case ALife::infl_fire: texture = "ui_inGame2_cop_triangle_Fire_"; break;
        case ALife::infl_acid: texture = "ui_inGame2_cop_triangle_Biological_"; break;
        case ALife::infl_psi: texture = "ui_inGame2_cop_triangle_Psy_"; break;
        default: NODEFAULT;
        }
        float hit_power = m_zone_cur_power[type];
        ALife::EHitType hit_type = m_zone_hit_type[type];
