...
CWeaponMagazined::CWeaponMagazined(ESoundTypes eSoundType) : CWeapon()
{
    ...
    m_bLockType = false;
	iMagSizeCurrent				= 0; // Новая GetState (+)
}

CWeaponMagazined::~CWeaponMagazined()    {                         }
void CWeaponMagazined::net_Destroy()     {inherited::net_Destroy();}
bool CWeaponMagazined::UseScopeTexture() {return bScopeIsHasTexture;}
...
bool CWeaponMagazined::TryReload()
{
    if (m_pInventory)
    {
        ...
        {
            SetPending(TRUE);
			SwitchState(eUnMisfire);  //Новая GetState (+)
        // (-)   SwitchState(eReload);
            return				true;
        }
...
//        *m_magazine.back().m_ammoSect)))								(-)
	...
	while(iAmmoElapsed < iMagSizeCurrent) //Новая GetState (+)
...
void CWeaponMagazined::OnAnimationEnd(u32 state)
{
	...
	//Новая GetState (+)
	case eUnMisfire: {bMisfire = false;--iAmmoElapsed;SwitchState(eIdle);}
