...
void CUIMotionIcon_soc::Init()
{
    ...
    AttachChild(&m_luminosity_progress_);
    xml_init.InitProgressBar(uiXml, "luminosity_progress", 0, &m_luminosity_progress_);
...
void CUIMotionIcon_soc::SetLuminosity_(float Pos)
{
    Pos = clampr(Pos, m_luminosity_progress_.GetRange_min(), m_luminosity_progress_.GetRange_max());
    m_luminosity = Pos;
}
...
void CUIMotionIcon_soc::Update()
{
    if (m_bchanged)
    {
        ...
        if (m_npc_visibility.size())
        {
            ...
            SetLuminosity_(m_luminosity_progress_.GetRange_min());
    }
    inherited::Update();
   ...
   
    {
        ...
        float cur_pos = m_luminosity_progress_.GetProgressPos();
        if (cur_pos != m_luminosity)
        {
            ...
            m_luminosity_progress_.SetProgressPos(cur_pos);
        }
    }
}
...
{
    if (g_pMotionIcon_soc)
        g_pMotionIcon_soc->SetActorVisibility_(who_id, value);
}

void CUIMotionIcon_soc::SetActorVisibility_(u16 who_id, float value)
{
    float v = float(m_luminosity_progress_.GetRange_max() - m_luminosity_progress_.GetRange_min());
    value *= v;
    value += m_luminosity_progress_.GetRange_min();
